 ...
 * @name $mmCourses
 */
.factory('$mmCourses', function($q, $mmSite, $log, $mmSitesManager, $filter,$translate, $timeout, $document, mmCoursesSearchPerPage, mmCoursesEnrolInvalidKey) {
     ...
     * @module mm.core.courses
     * @ngdoc method
     * @name $mmCourses#getDataCourse
     * @param {Object} course      Current objet course
     * @return {Object}         Object with the parsed data courses
     */
    self.getDataCourse = function (course) {
        var path = "img/custom/";
        var titleDate = "";
        var parseDate = "";
        switch(course.state) {
            case "sessioncomplete":
                path += "completed.png";
                titleDate = "mm.courses.enddate";
                var courseCompleteTS = parseInt(course.complete_ts);
                parseDate = self.parseDateCourse(courseCompleteTS);
                break;

            case "sessioninprogress":
                path += "inprogress.png";
                titleDate = "mm.courses.startdate";
                parseDate = self.parseDateCourse(course.startdate_ts);
                break;

            case "sessionnotyetstarted":
                path += "pending.png";  
                titleDate = "mm.courses.startdate";   
                parseDate = self.parseDateCourse(course.startdate_ts);           
                break;
   ...
   /**
     * Parsa date of course
     *
     * @module mm.core.courses
     * @ngdoc method
     * @name $mmCourses#parseDateCourse
     * @param {Number} day      Date in timestamp format
     * @return {String}        String with format: 'Day of week Month Year'. Sample 15 Enero 2018
     */
    self.parseDateCourse = function (day) {

        var date = new Date(day);
        var day = date.getDate();
        var month = $filter("translate")("mm.courses.month." + date.getMonth());
    ...
    /**
     * Check if score course is visible
     *
     * @module mm.core.courses
     * @ngdoc method
     * @name $mmCourses#checkVisibilityScore
     * @param {String} grade_nr      String with the course score
     * @return {Boolean}       Return true if score is filled. Otherwise, return false.
     */
    self.checkVisibilityScore = function(grade_nr) {
        if(grade_nr != "-" && grade_nr != "") {
            return true;
        }
        return false;
    }
    ...
    /**
     * Reset initial color fab tool bar
     *
     * @module mm.core.courses
     * @ngdoc method
     * @name $mmCourses#resetColorFabToolbar
     * @param none
     * @return none
     */
    self.resetColorFabToolbar = function() {
        $timeout(function() {
            var buttonsSorts = $document[0].body.querySelectorAll(".md-fab-toolbar-background");
            angular.forEach(buttonsSorts, function(button) {
                angular.element(button).css("background-color", "#9e9e9e");
            });
        });
    }
    ...
    /**
     * Set selected olor fab tool bar
     *
     * @module mm.core.courses
     * @ngdoc method
     * @name $mmCourses#setColorFabToolbarOptionSelected
     * @param none
     * @return none
     */
    self.setColorFabToolbarOptionSelected = function() {
        $timeout(function() {
            var buttonsSorts = $document[0].body.querySelectorAll(".md-fab-toolbar-background");           
            angular.element(buttonsSorts[1]).css("background-color", "#007982");
        });
    }

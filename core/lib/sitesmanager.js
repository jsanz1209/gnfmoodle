...
.factory('$mmSitesManager', function($http, $q, $mmSitesFactory, md5, $mmLang, $mmApp, $mmUtil, $mmEvents,
            ...
            $ionicHistory, $mmEvents, mmCoreEventLanguageChanged, mmCoreWSPrefix, mmCoreEventSiteUpdated, 
            mmCoreEventSiteAdded, mmCoreEventSessionExpired, mmCoreEventSiteDeleted, $mmText,
    ...
    self.newSite = function(siteurl, token, privateToken) {
        ...
        return candidateSite.fetchSiteInfo().then(function(infos) {
            if (isValidMoodleVersion(infos)) {
                ...
                var lang = infos.lang.replace("_", "-");

                //Set language returned for the user by moodle
                $mmLang.changeCurrentLanguage(lang).then(function() {
                    // Clear cached views.
                    $ionicHistory.clearCache();
                    $mmEvents.trigger(mmCoreEventLanguageChanged);
                 }).catch(function(error) {
                     var defaultLanguage = mmCoreConfigConstants.default_lang || 'es';
                     $mmLang.changeCurrentLanguage(defaultLanguage);
                 });
